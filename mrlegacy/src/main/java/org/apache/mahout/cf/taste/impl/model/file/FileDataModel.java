...
public class FileDataModel extends AbstractDataModel {
  ...
  public FileDataModel(File dataFile, boolean transpose, long minReloadIntervalMS, String delimiterRegex)
    ...
    Preconditions.checkArgument(dataFile.length() > 0L, "dataFile is empty"); //wxc 2015-01-30 10:43:22 第一次见这样判断文件是否为空的。
    ...
    while (firstLine.isEmpty() || firstLine.charAt(0) == COMMENT_CHAR) {
      iterator.next();
      firstLine = iterator.peek();//wxc pro 2015-01-30 11:05:28 这是只找出第一行来？
    }
  ...
  protected void reload() {//wxc pro 2015-01-30 10:51:16 为什么要reload？
    if (reloadLock.tryLock()) {
      try {
        delegate = buildModel(); //wxc pro 2015-01-30 11:08:21 为啥这里要再加一个delegate？ reload跟buildModel有什么必然的关联或联系？
  ...
  protected DataModel buildModel() throws IOException {
    ...
    long newLastModified = dataFile.lastModified(); //wxc 2015-01-30 10:52:25 这个接口看操作系统的理解，这里特别指文件管理系统。 对文件管理系统了解后， 就会很自然地找问，是不是File类是否有想要的支持。 不然， 现在大多是从基于字典的碰到，而不是基于原理的想要。
    ...
    FastByIDMap<FastByIDMap<Long>> timestamps = new FastByIDMap<FastByIDMap<Long>>();//wxc pro 2015-01-30 11:09:46 这个Map的接口层面上代表什么？又为什么是Fast的？ Id是用timestamp来定义的？

    if (hasPrefValues) { //wxc pro 2015-01-30 11:20:14 指哪些？
  ...
  private Iterable<File> findUpdateFilesAfter(long minimumLastModified) {
    ...
    File parentDir = dataFile.getParentFile();//wxc pro 2015-01-30 10:59:14 先回到父目录， 再找子文件，这样是要提供什么机制？为了什么方便？
  ...
  private long readLastUpdateFileModified() {
    ...
    for (File updateFile : findUpdateFilesAfter(0L)) {
      mostRecentModification = Math.max(mostRecentModification, updateFile.lastModified()); //wxc 2015-01-30 10:55:56 如果自己写， 很可能不用这个max函数， 而自己比较了。
    }
